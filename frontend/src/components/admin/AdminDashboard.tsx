...
import { Users, Building2, GraduationCap, UserCheck, UserX, TrendingUp, Calendar, FileText } from 'lucide-react';
...
type ActiveView = 'dashboard' | 'manage-students' | 'manage-faculty' | 'departments' | 'enroll-student';
...
const AdminDashboard: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case 'manage-faculty':
        return <ManageFaculty onBack={() => setActiveView('dashboard')} />;
      case 'departments':
        return <ManageDepartments onBack={() => setActiveView('dashboard')} />;
      case 'enroll-student':
        return <EnrollStudent onBack={() => setActiveView('dashboard')} />;
      default:
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              <button
                onClick={() => setActiveView('enroll-student')}
              ...
              <button
                onClick={() => setActiveView('manage-faculty')}
                className="w-full flex items-center justify-center space-x-2 border-2 border-gray-300 hover:border-gray-400 text-gray-700 font-medium py-3 px-4 rounded-lg transition-all"
              >
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              {[
                { action: 'New student enrolled', time: '2 mins ago', type: 'success' },
                { action: 'Low attendance alert', time: '15 mins ago', type: 'warning' },
                { action: 'Faculty added to CSE', time: '1 hour ago', type: 'info' },
                { action: 'Attendance marked', time: '2 hours ago', type: 'success' }
              ].map((activity, index) => (
